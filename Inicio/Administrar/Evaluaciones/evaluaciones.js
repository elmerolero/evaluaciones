function mostrarEvaluaciones()
{
	...
	$("#sec-evaluaciones").show();

	// Oculta las secciones no usadas
	$("#editable").hide();
...
function crearListaEvaluaciones()
{
	$.ajax({ type			: 'POST',
			 url	 		: 'Evaluaciones/evaluaciones.php',
			 dataType		: 'json',
			 encode			: true
		})
	.done(function(data){
		...
		else
		{
			...
			// Para el primer item
			$('.enlace-administracion:first').attr("href", data[0].Ruta_Administracion);
			...
			for(var i = 1; i < data.length; i++)
			{
				...
				$('.enlace-administracion:first').attr("href", data[i].Ruta_Administracion);
